...
import java.util.Vector;
 ...
 class ListPurchaseDialog extends JDialog implements ActionListener,ItemListener{
	...
	PurchaseMaster purchasemaster  = new PurchaseMaster();
        public ListPurchaseDialog() {
	setModal(true);
                setTitle("VIEW PURCHASE  DETAILS");
//		setLayout(null);
		setSize(940,550);
		setLocation(100,100);
               addleft(); 
...
//			getContentPane().setBackground(col);
	                ...
	                x1 = new JLabel("Select Invoice No. ");
...
//    		fillSupplierModel();
    		...
    		p.add(scrollPane);
    		scrollPane.setBorder(BorderFactory.createTitledBorder(BorderFactory.createEtchedBorder(),"Product wise purchase details"));
...
//    		p.setBackground(co);
	...
	dispose();
 	...
 	public  void convetarraylistintovectorofvector() {
 		...
 		for(int k=model.getRowCount()-1;k>=0;k--)
 		{
 			model.removeRow(k);
				...
				Vector v = new Vector();
				PurchaseDetail pd = (PurchaseDetail)purchasemaster.getItemlist().get(k);

				v.addElement(pd.getPinvoiceno());
				v.addElement(pd.getProduct().getPdescription());				
				v.addElement(pd.getProduct().getPqty()+"");
				v.addElement(pd.getProduct().getPrate()+"");
				total = total +(pd.getProduct().getPrate()); 
				v.addElement((pd.getProduct().getPqty()*pd.getProduct().getPrate()) + "");
 	...
 	public void itemStateChanged(ItemEvent ie)
 	{
 		...
 		String purchaseid = (String)jc.getSelectedItem();
 		purchasemaster = DataBase.getPurchases(purchaseid);
 		filldetails();
 	}
 	
public void  	fillcombo()
 	{
	ArrayList al =    DataBase.getPurchaseList();
	jc.removeAllItems();
	for(int k=0;k<al.size();k++)
		jc.addItem(al.get(k));
	if(jc.getItemCount()>0)
	{
	purchasemaster = DataBase.getPurchases((String)jc.getItemAt(0));
	filldetails();
	}
 	}
...
public void  	filldetails()
	{
		...
		tsuppierid.setText(purchasemaster.getPinvoiceno());
		texpenses.setText(purchasemaster.getPexpenses()+"");
		ttotal.setText(purchasemaster.getPtotal()+"");
		
		comboday.setSelectedItem(purchasemaster.getPday()+"");
		combomonth.setSelectedItem(purchasemaster.getPmonth()+"");
		comboyear.setSelectedItem(purchasemaster.getPear()+"");
